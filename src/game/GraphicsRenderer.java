...
public class GraphicsRenderer {
	...
	public void render(Texture image, Point position, Dimension dimension) {
		this.graphics.drawImage(image.asBufferedImage(), position.getX().intValue(), position.getY().intValue(), dimension.getWidth(), dimension.getHeight(), null);
	}
	...
	public void render(Cell cell, Point position, Dimension dimension) {
		...
		if (debugger.isShowingGrid()) {
			this.graphics.drawRect(position.getX().intValue(), position.getY().intValue(), dimension.getWidth(), dimension.getHeight());
		}
	...
	public void render(Message message) {
		...
		this.graphics.drawString(message.getText(), message.getPosition().getX().intValue(), message.getPosition().getY().intValue());
	}
	...
	public void render(Balloon balloon) {
		...
		final Rectangle bounds = new Rectangle(position.getX().intValue(), position.getY().intValue(), metrics.stringWidth(balloon.getMessageText()) + 10, metrics.getHeight() * 2);
